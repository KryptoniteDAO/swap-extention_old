...
use crate::handler::{
    change_owner, set_whitelist, swap_denom, update_pair_config, update_pair_max_spread,
    update_pair_status,
};
...
pub fn instantiate(
    ...
    store_config(deps.storage, &Config { owner: msg.owner })?;
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::UpdatePairMaxSpread {
            asset_infos,
            max_spread,
        } => {
            if asset_infos.len() != 2 {
        ...
        ExecuteMsg::SetWhitelist {
            caller,
            is_whitelist,
        } => set_whitelist(deps, info, caller, is_whitelist),
        ExecuteMsg::SwapDenom {
            from_coin,
            target_denom,
        } => swap_denom(deps, env, info, from_coin, target_denom),
    }
}
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::QueryIsSwapWhitelist { caller } => {
            to_binary(&query_is_swap_whitelist(deps, caller)?)
        }
